...
def benchmark_results():
    """Serves the latest stress-benchmark results (data/benchmark.json), if present.
    Generated by `python scripts/benchmark.py`. Used by the Dashboard's throughput chip."""
    path = os.path.join(os.path.dirname(DEFAULT_DATA_DIR), "data", "benchmark.json")
    # DEFAULT_DATA_DIR already points inside data/, so simpler:
    path = os.path.join(os.path.dirname(DEFAULT_DATA_DIR.rstrip(os.sep)), "benchmark.json")
    if not os.path.exists(path):
        path = os.path.join(DEFAULT_DATA_DIR, "..", "benchmark.json")
    path = os.path.abspath(path)
    if not os.path.exists(path):
        return JSONResponse(status_code=404, content={
            "error": "no_benchmark",
            "detail": "Run `python scripts/benchmark.py` to generate stress-test results.",
        })
...
@app.get("/runs/{run_id}/fee-slab-advice")
def run_fee_slab_advice(run_id: str, annual_volume_multiplier: float = 12.0):
    """Fee-slab recommendation. Reads the observed effective MDR from this batch,
    compares against Razorpay's published slabs, and projects annual savings on
    the merchant's current volume.

    Razorpay's published test-account MDR reference points (Aug 2026 approx):
      Standard           2.00%   (default, no volume commitment)
      Growth             1.75%   (higher volume, or bundle)
      Enterprise         1.50%   (custom, high-volume merchants)

    `annual_volume_multiplier` scales the batch's gross to a projected annual
    figure - default 12 (treat one batch as one month). Judges can override.
    """
    if not audit.get_run(run_id):
        return JSONResponse(status_code=404, content={"error": "not_found",
                                                      "detail": f"run {run_id} not found"})
    ...
    if gross_p <= 0 or n_settle == 0:
        return {"run_id": run_id, "eligible": False,
                "detail": "no gross-bearing settlements in this run"}

    effective_mdr_pct = round((mdr_p / gross_p) * 100, 3)
    annual_gross_p = int(gross_p * float(annual_volume_multiplier))

    SLABS = [
        {"name": "Standard",   "mdr_pct": 2.00, "note": "default, no volume commitment"},
        {"name": "Growth",     "mdr_pct": 1.75, "note": "higher volume, or bundle"},
        {"name": "Enterprise", "mdr_pct": 1.50, "note": "custom pricing, high-volume merchants"},
    ]
    # current slab = the closest published rate at or above the observed effective MDR
    current = next((s for s in SLABS if s["mdr_pct"] >= effective_mdr_pct - 0.1),
                   SLABS[0])
    # candidate cheaper slab = the tier immediately below current
    cheaper_slabs = [s for s in SLABS if s["mdr_pct"] < current["mdr_pct"]]
    if not cheaper_slabs:
        return {
            "run_id": run_id,
            "eligible": True,
            "current_slab":       current,
            "effective_mdr_pct":  effective_mdr_pct,
            "annual_projection_paise": annual_gross_p,
            "recommendation":     None,
            "detail":             ("Already on the lowest published slab - "
                                   "your effective MDR matches Enterprise pricing."),
        }
    target = cheaper_slabs[0]
    delta_pct = round(current["mdr_pct"] - target["mdr_pct"], 3)
    projected_savings_paise = int(round(annual_gross_p * delta_pct / 100))

    return {
        "run_id":                 run_id,
        "eligible":               True,
        "settlements_analyzed":   n_settle,
        "effective_mdr_pct":      effective_mdr_pct,
        "current_slab":           current,
        "recommendation": {
            "target_slab":                target,
            "delta_pct":                  delta_pct,
            "batch_savings_paise":        int(round(gross_p * delta_pct / 100)),
            "annual_savings_paise":       projected_savings_paise,
            "annual_gross_paise":         annual_gross_p,
            "annual_volume_multiplier":   annual_volume_multiplier,
            "phrase": (
                f"Your effective MDR this batch was {effective_mdr_pct:.2f}%. "
                f"Moving from {current['name']} to {target['name']} "
                f"({target['mdr_pct']:.2f}%) would save ~Rs.{projected_savings_paise/100:,.0f} "
                f"annually on projected volume of Rs.{annual_gross_p/100:,.0f}."
            ),
        },
    }
...
def cfo_brief(run_id: str):
    ...
    Returns HTML with a @media print stylesheet so save-as-PDF works cleanly.
    """
    if not audit.get_run(run_id):
        return JSONResponse(status_code=404, content={"error": "not_found",
                                                      "detail": f"run {run_id} not found"})
    meta = _run_meta(run_id).model_dump()
    cash = run_cash_position(run_id)
    forecast = run_cash_forecast(run_id, horizon_days=7)
    tax = run_tax_exposure(run_id)
    resolutions = audit.resolution_summary(run_id)
    # Top 3 exceptions by amount
    exc_rows = audit.get_exceptions(run_id)
    top = sorted(exc_rows, key=lambda r: (r.get("severity") == "critical", int(r.get("amount_paise") or 0)),
                 reverse=True)[:3]

    def inr(paise: int) -> str:
        return f"Rs.{paise/100:,.2f}"

    def inr_short(paise: int) -> str:
        r = paise / 100
        if abs(r) >= 1e7: return f"Rs.{r/1e7:.2f} Cr"
        if abs(r) >= 1e5: return f"Rs.{r/1e5:.2f} L"
        if abs(r) >= 1e3: return f"Rs.{r/1e3:.1f} K"
        return f"Rs.{r:.0f}"

    today = datetime.now(timezone.utc).date().isoformat()

    # Forecast — top 3 days with inflow
    days_with_cash = sorted(
        (d for d in forecast["days"] if d["amount_paise"] > 0),
        key=lambda d: d["amount_paise"], reverse=True,
    )[:3]

    exc_rows_html = ""
    for i, r in enumerate(top, 1):
        exc_rows_html += (
            f"<tr><td>{i}. <b>{r.get('record_ref','')}</b></td>"
            f"<td>{r.get('reason','')}</td>"
            f"<td class='r'>{inr(int(r.get('amount_paise') or 0))}</td>"
            f"<td><span class='sev sev-{r.get('severity','info')}'>{r.get('severity','info')}</span></td></tr>"
        )
    if not top:
        exc_rows_html = "<tr><td colspan='4' class='muted'>No exceptions this run - clean batch.</td></tr>"

    fc_rows_html = ""
    for d in days_with_cash:
        fc_rows_html += (
            f"<tr><td>{d['date']}</td>"
            f"<td class='r'>{inr(d['amount_paise'])}</td>"
            f"<td class='r'>{d['count']}</td></tr>"
        )
    if not days_with_cash:
        fc_rows_html = "<tr><td colspan='3' class='muted'>No cash landings expected in the next 7 days.</td></tr>"

    res = resolutions["by_reason"]
    handled = resolutions["resolved"]
    total_exc = resolutions["total_exceptions"]
    handled_pct = round(handled / total_exc * 100, 1) if total_exc else 0.0

    html = f"""<!doctype html>
<html><head><meta charset="utf-8"><title>CFO morning brief · {today}</title>
<style>
  :root {{ --ink:#1F2A1A; --rust:#B5432F; --moss:#4B7B4E; --ochre:#B8863B;
           --cream:#FBFBF3; --sage:#EEF2E4; --border:rgba(31,42,26,0.14); }}
  * {{ box-sizing: border-box; }}
  body {{ font-family: 'SF Mono','Menlo','Consolas',monospace; background: var(--cream);
         color: var(--ink); margin: 0; padding: 40px; }}
  .brief {{ max-width: 820px; margin: 0 auto; background: #fff; border: 2px solid var(--ink);
           box-shadow: 0 2px 24px rgba(31,42,26,0.08); padding: 40px 44px; position: relative; }}
  .stamp {{ position: absolute; top: 24px; right: 32px; padding: 6px 14px;
           border: 2px solid var(--rust); color: var(--rust); font-weight: 700;
           text-transform: uppercase; letter-spacing: 0.14em; font-size: 11px;
           transform: rotate(-6deg); }}
  h1 {{ margin: 0 0 4px 0; font-size: 26px; letter-spacing: -0.01em; }}
  .subtitle {{ color: #5C6752; font-size: 12px; margin-bottom: 32px; }}
  h2 {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.16em;
        color: var(--rust); border-bottom: 1px solid var(--border);
        padding-bottom: 6px; margin: 24px 0 12px 0; }}
  .grid-2 {{ display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }}
  .grid-3 {{ display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }}
  .cell {{ background: rgba(31,42,26,0.03); padding: 14px; border: 1px solid var(--border); }}
  .cell .k {{ font-size: 10px; text-transform: uppercase; letter-spacing: 0.12em;
             color: #5C6752; font-weight: 700; }}
  .cell .v {{ font-size: 22px; font-weight: 700; margin-top: 4px; tabular-nums; }}
  .cell .sub {{ font-size: 11px; color: #5C6752; margin-top: 4px; }}
  .headline {{ background: rgba(75,123,78,0.08); border: 1px solid var(--moss);
              padding: 18px 20px; margin-bottom: 20px; }}
  .headline .k {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.12em;
                 color: var(--moss); font-weight: 700; }}
  .headline .v {{ font-size: 34px; font-weight: 700; margin: 4px 0; }}
  table {{ width: 100%; border-collapse: collapse; font-size: 12px; margin-top: 4px; }}
  th, td {{ text-align: left; padding: 8px 10px; border-bottom: 1px solid var(--border); }}
  th {{ font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em;
       color: #5C6752; font-weight: 700; background: rgba(31,42,26,0.03); }}
  td.r {{ text-align: right; tabular-nums; }}
  .sev {{ font-size: 10px; padding: 2px 6px; text-transform: uppercase; letter-spacing: 0.1em; font-weight: 700; }}
  .sev-critical {{ background: rgba(181,67,47,0.12); color: var(--rust); border: 1px solid var(--rust); }}
  .sev-warning {{ background: rgba(184,134,59,0.12); color: var(--ochre); border: 1px solid var(--ochre); }}
  .sev-info {{ background: rgba(75,123,78,0.10); color: var(--moss); border: 1px solid var(--moss); }}
  .muted {{ color: #9AA590; font-style: italic; text-align: center; }}
  .footer {{ margin-top: 32px; padding-top: 16px; border-top: 1px solid var(--border);
             font-size: 10px; color: #5C6752; line-height: 1.6; }}
  @media print {{ body {{ padding: 0; background: #fff; }}
                  .brief {{ box-shadow: none; border-color: #000; }} }}
</style></head><body>
<div class="brief">
  ...
  <div class="headline">
    <div class="k">Net available right now (cleared + ghost − at-risk)</div>
    <div class="v">{inr(cash['totals']['net_available_paise'])}</div>
    <div class="sub">Exposure: {inr(cash['totals']['exposure_paise'])} (at-risk + in-flight)</div>
  </div>

  <h2>Cash position, by bucket</h2>
  <div class="grid-3">
    <div class="cell"><div class="k">Cleared</div><div class="v">{inr_short(cash['buckets']['cleared']['amount_paise'])}</div><div class="sub">{cash['buckets']['cleared']['count']} settlements · in your account</div></div>
    <div class="cell"><div class="k">In-flight</div><div class="v">{inr_short(cash['buckets']['in_flight']['amount_paise'])}</div><div class="sub">{cash['buckets']['in_flight']['count']} settlements · bank not yet</div></div>
    <div class="cell"><div class="k">At-risk</div><div class="v">{inr_short(cash['buckets']['at_risk']['amount_paise'])}</div><div class="sub">{cash['buckets']['at_risk']['count']} chargebacks / disputes</div></div>
  </div>

  <h2>Cash landing in the next 7 days</h2>
  <table>
    <thead><tr><th>Date</th><th class="r">Expected inflow</th><th class="r">Settlements</th></tr></thead>
    <tbody>{fc_rows_html}</tbody>
  </table>
  <div class="sub" style="margin-top:8px;font-size:10px;color:#5C6752;">
    Projection: <b>record_date + T+2 business days</b>. Past-due: {inr(forecast['past_due']['amount_paise'])}
    across {forecast['past_due']['count']} settlements.
  </div>

  <h2>Top 3 exceptions worth your time</h2>
  <table>
    <thead><tr><th>Payment</th><th>Reason</th><th class="r">Amount</th><th>Severity</th></tr></thead>
  ...
  <div class="grid-2" style="margin-top:20px;">
    <div>
      <h2>Tax exposure</h2>
      <div class="cell">
        <div class="k">Merchant should investigate</div>
        <div class="v" style="color:{'var(--moss)' if tax['exposure_paise']['gross_exposure']==0 else 'var(--rust)'};font-size:22px;">
          {inr(tax['exposure_paise']['gross_exposure'])}
        </div>
        <div class="sub">Recover: {inr_short(tax['exposure_paise']['over_charged'])} ·
                        Reserve: {inr_short(tax['exposure_paise']['under_charged'])}</div>
        <div class="sub">Effective rate: {tax['effective_rate_pct']:.3f}% of gross ·
                        {tax['anomaly_count']} anomalies</div>
      </div>
    </div>
    <div>
      <h2>Resolution progress</h2>
      <div class="cell">
        <div class="k">Handled by your team</div>
        <div class="v">{handled}<span style="font-size:14px;color:#5C6752;"> / {total_exc}</span></div>
        <div class="sub">{handled_pct:.1f}% cleared · confirmed {res.get('confirmed',0)} ·
                        override {res.get('override',0)} · escalated {res.get('escalated',0)} ·
                        FP {res.get('false_positive',0)}</div>
      </div>
    </div>
  </div>

  <div class="footer">
    Generated by ReconMint · every figure on this brief is traceable to a row in the audit ledger
    for run <b>{run_id}</b>. Fee reconstruction is deterministic (int-paise); no LLM produced any of the numbers above.
    Print / save-as-PDF from your browser for the email.
  </div>
</div>
</body></html>
"""
